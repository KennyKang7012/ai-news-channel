...
### 問題 3｜`verify_build.sh` 的字數檢查（`wc -w`）不支援中文（🔴 高風險）
...
### 問題 4｜Twitter 字數檢查依賴 bash 字串長度，是隱性的 locale 相依風險（🟡 中低風險，目前尚未爆發）
...
### 問題 5｜`verify_build.sh` 完全沒有檢查第 4 則貼文（Instagram）（🟡 中風險）
...
### 問題 6｜`developer.md` 的來源數量硬性下限（3 則）低於今天 CTO brief 的實際要求（5 則，≥2 一手來源）（🟢 低風險，僅需人工留意）
...
| 優先度 | 項目 | 原因 |
|---|---|---|
| 🔴 1 | 問題 2：圖卡檔名對齊 | 不修正，今天這輪 Developer 產出後會被誤判 FAIL |
| 🔴 2 | 問題 3：中文字數驗證邏輯 | 不修正，文章一定會被誤判字數不足 |
| 🟡 3 | 問題 5：Instagram 自動驗證缺口 | 本輪可先人工複查代替，不阻斷 dry run |
| 🟢 4 | 問題 4：bash locale 相依風險 | 目前環境恰好正常，建議記錄，未來重構 script 時一併處理 |
| 🟢 5 | 問題 6：來源數量下限落差 | 人工留意即可，不需改檔案 |
